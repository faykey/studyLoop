...
const Navbar = () => {
  ...
  const isAuthenticated = localStorage.getItem("token"); // Or use your auth check logic
    ...
    <nav className="fixed top-0 w-full bg-white z-50 shadow-md">
      <div className="max-w-7xl mx-auto px-4 py-3 flex justify-between items-center relative z-50">
        <div className="flex items-center gap-3">
          ...
          {/* 👇 Conditionally render logo only if user is NOT authenticated */}
          {!isAuthenticated && (
            <NavLink to="/" className="md:ml-2 ml-0">
              <img src={logo} alt="StudyLoop Logo" className="h-10 w-auto" />
            </NavLink>
          )}
        </div>
        ...
        <div className="hidden lg:flex gap-6 items-center mx-auto mr-20">
          {navItems.map((item) => (
            <NavLink
              ...
              to={item.path}
              className={({ isActive }) =>
                `flex items-center gap-2 font-medium text-sm px-3 py-2  rounded-md transition-colors ${isActive
      ...
      {/* Mobile Nav */}
      <div
        className={`fixed top-[64px] left-0 w-full border-t border-[hsla(220,13%,91%,1)] bg-white shadow-md overflow-hidden transition-all duration-500 ease-in-out z-40 ${isOpen ? "max-h-[500px]" : "max-h-0"
          }`}
      >
        <div className="flex flex-col px-4 py-4 gap-3">
          {navItems.map((item, index) => (
            <NavLink
              ...
              className={({ isActive }) =>
                `flex items-center gap-3 px-4 py-3 font-medium text-base rounded-lg ${index === 0 ? "mt-2 pt-4" : ""
                } ${isActive
                  ? "bg-green-600 text-white"
