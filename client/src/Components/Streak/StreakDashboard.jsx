...
import { getUserStreakDetails } from "../../utils/api";

const STREAK_GOALS = [
  { title: "Wildfire", days: 3, icon: "🔥", color: "from-[hsla(27,96%,61%,1)] to-[hsla(0,84%,60%,1)]" },
  { title: "Scholar", days: 7, icon: "📘", color: "from-[hsla(213,94%,68%,1)] to-[hsla(271,91%,65%,1)]" },
  { title: "Achiever", days: 14, icon: "🥉", color: "from-yellow-300 to-yellow-600" },
  { title: "Legend", days: 30, icon: "🏅", color: "from-orange-400 to-red-500" },
  { title: "Master", days: 100, icon: "🌟", color: "from-blue-400 to-blue-700" },
];
    ...
    <div className="bg-gradient-to-r from-[#f3f7fd] to-[#fefeff] min-h-screen flex flex-col items-center py-10 px-4 space-y-6">
      ...
      <div className="flex gap-4 w-full max-w-md">
        ...
        <div className="flex-1 bg-white border-2 border-[hsla(141,84%,93%,1)] shadow-md rounded-xl px-4 py-5 text-center">
          ...
          <div className="text-lg font-semibold">{streak.totalStudyDays || 0}</div>
      ...
      {nextGoal && (
        <div className="bg-white w-full max-w-md rounded-xl p-4 shadow-md border-2 border-[hsla(141,84%,93%,1)]">
          <div className="flex justify-between items-start mb-1 text-sm font-medium text-gray-700">
            <div>
              <p>Next Goal</p>
              <p className="text-xs text-gray-500 mb-2">{nextGoal.title} Badge</p>
            </div>
          ...
          <div className="w-full bg-gray-200 h-2 rounded-full overflow-hidden mb-1">
            <div
              className="bg-gradient-to-r from-[hsla(142,71%,45%,1)] to-[hsla(142,71%,22%,1)] rounded-full h-full"
              style={{ width: `${percentToNext}%` }}
            ></div>
          </div>
          <div className="text-xs text-gray-500 flex justify-between">
            <span>{daysLeft} days to go!</span>
            <span className="text-orange-500 font-medium">{percentToNext}%</span>
          </div>
        </div>
      )}
      ...
      <div className="bg-white w-full max-w-md rounded-xl p-6 shadow-md border-2 border-[hsla(141,84%,93%,1)]">
        ...
        <div className="space-y-3">
          {achievements.map((ach, index) => (
            <div
              key={index}
              className={`rounded-2xl px-4 py-3 flex justify-between items-center ${ach.earned
      ...
      <div className="shadow-[hsla(0,0%,0%,0.1)] bg-gradient-to-r from-[hsla(142,71%,45%,1)] to-[hsla(142,71%,22%,1)] text-white text-center px-6 py-8 rounded-xl shadow-md max-w-md w-full">
        ...
        <div className="text-sm mt-2">Tomorrow will be day {currentStreak + 1}</div>
      </div>
    </div>
  );
};
